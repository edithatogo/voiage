#!/usr/bin/env python3
"""
Performance Benchmarking for voiage - Establishing NumPy Backend Baselines
...
Usage:
    python performance_benchmark.py [--sizes small,medium,large] [--iterations 10]
"""
...
try:
    ...
    from voiage.methods.basic import evpi as evpi_method, evppi as evppi_method
    from voiage.methods.sample_information import evsi as evsi_method, enbs as enbs_method
    print("✅ Successfully imported voiage modules")
except ImportError as e:
    print(f"❌ Error importing voiage modules: {e}")
    sys.exit(1)

# Import backends for backend-specific testing
try:
...
def benchmark_evpi(data: Tuple, monitor: PerformanceMonitor, iterations: int = 5) -> BenchmarkResult:
    """Benchmark EVPI calculation."""
    ...
    for _ in range(iterations):
        ...
        # Calculate EVPI using NumPy backend
...
def benchmark_evppi(data: Tuple, monitor: PerformanceMonitor, iterations: int = 5) -> BenchmarkResult:
    """Benchmark EVPPI calculation using DecisionAnalysis."""
    net_benefit_array, param_names, option_names = data
    results = []
    ...
    for _ in range(iterations):
        ...
        value_array = ValueArray.from_2d_array(net_benefit_array, option_names, param_names)
        ...
        evppi_result = analysis.evppi(param_samples, parameters_of_interest=[param_names[0]])
...
def benchmark_evsi(data: Tuple, monitor: PerformanceMonitor, iterations: int = 5) -> BenchmarkResult:
    """Benchmark EVSI calculation using DecisionAnalysis."""
    net_benefit_array, param_names, option_names = data
    results = []
    ...
    for _ in range(iterations):
        ...
        # Create DecisionAnalysis instance and calculate EVSI
        value_array = ValueArray.from_2d_array(net_benefit_array, option_names, param_names)
        analysis = DecisionAnalysis(value_array, sample_size=len(net_benefit_array))
        
        # Create sample information for EVSI
        sample_data = {
            param_names[0]: np.random.randn(len(net_benefit_array)),
            param_names[1]: np.random.randn(len(net_benefit_array))
        }
        evsi_result = analysis.evsi(sample_data)
...
def benchmark_enbs(data: Tuple, monitor: PerformanceMonitor, iterations: int = 5) -> BenchmarkResult:
    """Benchmark ENBS calculation using DecisionAnalysis."""
    net_benefit_array, param_names, option_names = data
    results = []
    ...
    for _ in range(iterations):
        ...
        # Create DecisionAnalysis instance and calculate ENBS
        value_array = ValueArray.from_2d_array(net_benefit_array, option_names, param_names)
        analysis = DecisionAnalysis(value_array, sample_size=len(net_benefit_array))
        
        # Create parameter set for ENBS
        param_set = ParameterSet(
            param_names,
            [np.random.randn(100) for _ in param_names],
            ["uniform" for _ in param_names]
        )
        enbs_result = analysis.enbs(param_set)
...
def print_summary(results: Dict[str, List[BenchmarkResult]]):
    ...
    if "small" in results and "medium" in results and "large" in results:
        small_evpi = next(r.execution_time for r in results["small"] if r.method_name == "EVPI")
        medium_evpi = next(r.execution_time for r in results["medium"] if r.method_name == "EVPI")
        large_evpi = next(r.execution_time for r in results["large"] if r.method_name == "EVPI")
        
        print(f"   EVPI Time Scaling:")
        print(f"      Small → Medium: {medium_evpi/small_evpi:.1f}x")
        print(f"      Medium → Large: {large_evpi/medium_evpi:.1f}x")
        print(f"      Small → Large: {large_evpi/small_evpi:.1f}x")
