...
Exposed as explicit execution-adapter placeholders (`voiage.parallel.is_placeholder_execution_adapter("fpga")`) that fail with `NotImplementedError` until real runtimes are added.
