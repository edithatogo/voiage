...
Benchmark status is evidence-gated: CPU baselines are committed, while
device-backed speedup remains pending until reviewed production packets exist.
The current Apple Metal track has established the committed CPU baselines;
device-backed comparison remains separately gated.
...
Every evidence packet carries a `scalar_cpu_baseline` and a
...
Apple deployment requirements include macOS Apple Silicon hosts for build and
...
The Apple Metal track validates CPU reference and Metal/MPS packets for workload
hash, warm-up, timing, throughput, CPU baseline, result parity, and fallback.
When Apple Silicon is unavailable, the handoff records a blocked status. CPU
fallback remains authoritative; setup or MPS visibility alone is not an Apple
production-speedup claim.

## Discrete GPU evidence

The discrete GPU track requires CUDA device metadata, workload hashes, warm-up
iterations, repeated timing, throughput, CPU comparison, transfer overhead,
compile overhead, result parity, and CPU fallback. Colab or cloud quota/runtime
