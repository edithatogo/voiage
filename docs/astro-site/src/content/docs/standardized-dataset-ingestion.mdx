...
workloads. Cache namespace directories are also checked before every cache
operation, so a symlink cannot redirect a materialization outside the configured
cache root.
