---
title: Backends
description: Computational backends in voiage
---
...
- **JAX** is used when backend-accelerated execution is requested and the runtime supports it
- **Apple Metal** is available as an optional internal backend on macOS Apple Silicon when PyTorch provides an MPS device
- Public APIs should continue to work when JAX is unavailable; accelerated paths are optional, not mandatory
...
The canonical public data structures remain xarray-backed regardless of which backend is selected.
