...
| Research-use evidence | The immutable same-author `vop_poc_nz` integration documents use in a health-economic research workflow; the fixed-seed example supplies reproducible supporting material. The [independent validation protocol](joss-independent-validation.md) and issue #471 seek the non-author activity JOSS identifies as a strong positive signal | Evidence is specific and public, but screening remains an editorial judgement; independent use is not claimed |
...
| R | `cargo build --manifest-path rust/Cargo.toml --release --locked --package voiage-ffi`, then `R CMD build r-package/voiageR`, `R CMD check --as-cran --no-manual voiageR_*.tar.gz`, and an installed smoke test with `VOIAGE_FFI_LIBRARY` set to the platform library | Installed package calls the separately built Rust EVPI library; `R CMD check` passes; Linux/macOS/Windows native smoke matrix is configured | Secondary binding; CRAN/r-universe review is independent of JOSS |
| Julia | `cargo build --manifest-path rust/Cargo.toml --release --locked --package voiage-ffi`, then `VOIAGE_FFI_LIBRARY=<platform-library> julia --project=bindings/julia -e 'using Pkg; Pkg.instantiate(); Pkg.test()'` | The test suite calls the separately built Rust EVPI library and checks numerical fixtures | Secondary binding; a Julia artifact/JLL is required before standalone General installation |
