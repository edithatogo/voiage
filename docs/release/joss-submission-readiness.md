...
| JOSS manuscript structure | All contracted sections are substantive and ordered; the current local deterministic count is 1,583, inside the repository's 1,568–1,632 target and JOSS's 750–1,750 range | Local contract ready; hosted confirmation pending |
...
| Reproducible JOSS PDF | The preceding source passed Open Journals build and visual review. The round-nine evidence-led revision changes `paper.md` and therefore requires a fresh hosted build, artifact digest, Textstat report, and page-by-page inspection | Pending current-source evidence |
