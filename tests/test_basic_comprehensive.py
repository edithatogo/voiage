"""Test for voiage.methods.basic module to increase coverage."""
...
class TestBasicMethods:
    """Test the basic VOI methods to increase coverage."""
    ...
    def test_evpi_basic_calculation(self):
        ...
        nb_values = np.array([
            [100, 150],  # Sample 1: Strategy 2 is better (150 vs 100)
            [200, 180],  # Sample 2: Strategy 1 is better (200 vs 180)
            [120, 140]   # Sample 3: Strategy 2 is better (140 vs 120)
        ], dtype=np.float64)
    ...
    def test_evpi_with_population_scaling(self):
        ...
        # Create test data
        nb_values = np.random.rand(100, 3) * 1000  # 100 samples, 3 strategies
    ...
    def test_evpi_identical_strategies(self):
        ...
        nb_values = np.tile(identical_values, (50, 1))  # 50 samples, all identical
        value_array = ValueArray.from_numpy(nb_values, ["Strategy A", "Strategy B", "Strategy C"])
    ...
    def test_evpi_one_sample(self):
        ...
        # Single sample - no variance, so EVPI should be 0
        nb_values = np.array([[100.0, 150.0, 120.0]], dtype=np.float64)  # 1 sample, 3 strategies
        value_array = ValueArray.from_numpy(nb_values, ["Strategy A", "Strategy B", "Strategy C"])

        result_evpi = evpi(value_array)

        # Expected max NB per sample = 150 (max of [100, 150, 120])
        # Expected NB per strategy = [100, 150, 120] (same as sample since only 1)
        # Max of expected NB = 150
        # EVPI = 150 - 150 = 0
        assert abs(result_evpi) < 1e-10
    ...
    def test_evppi_multiple_params(self):
        ...
        # Create test data
        nb_values = np.random.rand(100, 2) * 1000  # 100 samples, 2 strategies
        value_array = ValueArray.from_numpy(nb_values, ["Strategy A", "Strategy B"])
    ...
    def test_evppi_input_validation(self):
        ...
        nb_values = np.random.rand(100, 2).astype(np.float64) * 1000
        value_array = ValueArray.from_numpy(nb_values, ["Strategy A", "Strategy B"])
    ...
    def test_evppi_no_param_uncertainty(self):
        ...
        nb_values = np.random.rand(50, 2) * 1000
        value_array = ValueArray.from_numpy(nb_values, ["Strategy A", "Strategy B"])

        # Create parameter samples with constant values (no uncertainty)
        params = {
            "param1": np.full(50, 0.5),  # All samples have same value
            "param2": np.full(50, 100.0)  # All samples have same value
        }
        parameter_set = ParameterSet.from_numpy_or_dict(params)
    ...
    def test_evppi_linear_relationship(self):
        ...
        n_samples = 100
        param_values = np.linspace(0, 1, n_samples)  # Parameter values from 0 to 1
        ...
        nb_values = np.column_stack([strategy_a_nb, strategy_b_nb])
    ...
    def test_evppi_edge_case_single_parameter_sample(self):
        ...
        # Create test data with the correct dtype
        ...
        # With only a single parameter sample, EVPPI calculation will not be meaningful
        # but should not crash
        try:
            result_evppi = evppi(value_array, parameter_set, ["param1"])
            # Result should be valid (might be 0 or very small due to only 1 sample)
            assert isinstance(result_evppi, float)
            assert result_evppi >= 0
        except Exception as e:
            # It might fail due to insufficient samples for regression, which is ok
            assert "regression" in str(e).lower() or "least squares" in str(e).lower() or "nb_array" in str(e).lower()
