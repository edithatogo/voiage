...
"""
Unit tests for the basic VOI methods (EVPI, EVPPI) in pyvoi.methods.basic.
"""
...
from pyvoi.exceptions import InputError, CalculationError, DimensionMismatchError
...
def test_evpi_calculation_simple(sample_nb_data_array_2strat: np.ndarray):
    """
    Test EVPI calculation with a simple, known scenario.
    Uses fixture `sample_nb_data_array_2strat` from conftest.py.
    """
    # Data from fixture:
    # nb_data_evpi = np.array([
    #     [100, 105], [110, 100], [90, 110], [120, 100], [95, 115]
    # ])
    # Expected: E[max(NB)] = mean([105, 110, 110, 120, 115]) = 112
    #           max(E[NB_A], E[NB_B]) = max(mean([100,110,90,120,95]), mean([105,100,110,100,115]))
    #                                = max(103, 106) = 106
    # EVPI = 112 - 106 = 6.0
    expected_evpi = 6.0
...
def test_evpi_with_netbenefitarray_input(sample_net_benefit_array_2strat: NetBenefitArray):
...
def test_evpi_invalid_inputs():
    ...
    with pytest.raises(InputError, match="cannot be empty"):
        evpi(np.array([[],[]], dtype=DEFAULT_DTYPE).reshape(0,2)) # Empty array
    ...
    with pytest.raises(InputError, match="To calculate population EVPI, 'population' and 'time_horizon' must be provided"):
...
pytestmark_evppi = pytest.mark.skipif(not SKLEARN_AVAILABLE, reason="scikit-learn not available, skipping EVPPI tests")
...
def test_evppi_basic_properties(evppi_test_data_simple):
    ...
    assert calculated_evppi >= -1e-9, f"EVPPI should be non-negative, got {calculated_evppi}."
...
def test_evppi_input_types(evppi_test_data_simple):
    ...
    assert np.isclose(evppi_np_1d, evppi_np_2d), "EVPPI with 1D vs 2D param array differs."
...
def test_evppi_population_scaling(evppi_test_data_simple):
    ...
    assert np.isclose(calculated_pop_evppi, expected_pop_evppi, rtol=1e-2 if per_decision_evppi > 1e-6 else 0, atol=1e-3), \
...
def test_evppi_invalid_inputs(evppi_test_data_simple):
    ...
    with pytest.raises(InputError, match="nb_array must be a NumPy array or NetBenefitArray"):
        evppi("not an array", p_samples_valid) # type: ignore

    with pytest.raises(InputError, match="parameter_samples must be a NumPy array, PSASample, or Dict"):
    ...
    with pytest.raises(InputError, match="n_regression_samples .* not exceed total samples"):
        evppi(nb_values, p_samples_valid, n_regression_samples=len(p_samples_valid)+10)

    # Population scaling errors (similar to EVPI)
    with pytest.raises(InputError, match="To calculate population EVPPI, 'population' and 'time_horizon' must be provided"):
...
def test_evppi_perfect_parameter(evppi_test_data_simple):
    """
    Test EVPPI when the parameter of interest perfectly explains the decision uncertainty.
