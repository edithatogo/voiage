...
def test_core_metrics_regression() -> None:
    ...
    actual = {
        ...
        "evppi": float(DecisionAnalysis(nb_array=_value_array(), parameter_samples=_parameter_set()).evppi(n_regression_samples=40)),
        "evsi": float(evsi(model_func=_model, psa_prior=_parameter_set(), trial_design=_trial_design(), method="efficient")),
    }
