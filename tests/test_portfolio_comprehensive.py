...
class TestPortfolioVOI:
    ...
    def test_portfolio_voi_basic(self) -> None:
    ...
    def test_portfolio_voi_no_budget_constraint(self) -> None:
    ...
    def test_portfolio_voi_empty_studies_skipped(self) -> None:
        """Test portfolio_voi with an empty list of studies."""
        pytest.skip(
            "PortfolioSpec doesn't allow empty studies, so this test case is not applicable"
        )

    def test_portfolio_voi_integer_programming(self) -> None:
        ...
        try:
            ...
            assert isinstance(result["selected_studies"], list)
            assert isinstance(
                result["total_value"], (float, int, np.floating, np.integer, np.number)
            )
            assert isinstance(
                result["total_cost"], (float, int, np.floating, np.integer, np.number)
            )
            assert isinstance(result["method_details"], str)
    ...
    def test_portfolio_voi_invalid_inputs(self) -> None:
        ...
        with pytest.raises(
            InputError, match="`portfolio_specification` must be a PortfolioSpec"
        ):
            portfolio_voi(
        ...
        with pytest.raises(
            InputError, match="`study_value_calculator` must be a callable"
        ):
            portfolio_voi(
                portfolio_specification=portfolio_spec,
        ...
        with pytest.raises(
            VoiageNotImplementedError, match="not recognized or implemented"
        ):
            portfolio_voi(
                portfolio_specification=portfolio_spec,
                study_value_calculator=simple_value_calculator,
    ...
    def test_portfolio_voi_dynamic_programming_method(self) -> None:
        """Test portfolio_voi with the exact dynamic programming method."""
        ...
        assert [s.name for s in result["selected_studies"]] == ["Study 1"]
        assert result["total_cost"] == 50000
        assert result["total_value"] == 100000.0
        assert result["method_details"]["algorithm"] == "dynamic_programming"

    def test_portfolio_voi_dynamic_programming_outperforms_greedy(self) -> None:
        """Test DP finds the optimal subset when greedy ratio sorting misses it."""
        ...
        studies = [
            PortfolioStudy("High Ratio", design, cost=10),
            PortfolioStudy("Best Bundle A", design, cost=50),
            PortfolioStudy("Best Bundle B", design, cost=50),
        ]
        portfolio_spec = PortfolioSpec(studies=studies, budget_constraint=100)

        greedy_result = portfolio_voi(
            portfolio_specification=portfolio_spec,
            study_value_calculator=fixed_value_calculator,
            optimization_method="greedy",
        )
        dp_result = portfolio_voi(
            portfolio_specification=portfolio_spec,
            study_value_calculator=fixed_value_calculator,
            optimization_method="dynamic_programming",
        )

        assert greedy_result["total_value"] == 160.0
        assert dp_result["total_value"] == 200.0
        assert [s.name for s in dp_result["selected_studies"]] == [
            "Best Bundle A",
            "Best Bundle B",
        ]
        assert dp_result["total_cost"] == 100.0

    def test_portfolio_voi_dynamic_programming_dependency_groups(self) -> None:
        """Test DP discounts studies that target already-selected uncertainty groups."""
        ...
        design = TrialDesign([DecisionOption("Treatment A", 100)])
        studies = [
            PortfolioStudy("Shared A", design, cost=40),
            PortfolioStudy("Shared B", design, cost=40),
            PortfolioStudy("Independent", design, cost=40),
        ]
        portfolio_spec = PortfolioSpec(studies=studies, budget_constraint=80)
        ...
        result = portfolio_voi(
            ...
            optimization_method="dynamic_programming",
            dependency_groups={
                "Shared A": "shared_parameter",
                "Shared B": "shared_parameter",
                "Independent": "independent_parameter",
            },
            dependency_discount=0.0,
        )

        assert [s.name for s in result["selected_studies"]] == [
            "Shared A",
            "Independent",
        ]
        assert result["total_value"] == 180.0
        assert result["method_details"]["dependency_groups_used"] is True

    def test_portfolio_voi_dynamic_programming_input_validation(self) -> None:
        """Test DP-specific option validation."""
        ...
        portfolio_spec = PortfolioSpec(
            studies=[PortfolioStudy("Study 1", design, cost=50000)],
            budget_constraint=100000,
        )

        with pytest.raises(InputError, match="dependency_groups"):
            portfolio_voi(
                portfolio_specification=portfolio_spec,
                study_value_calculator=simple_value_calculator,
                optimization_method="dynamic_programming",
                dependency_groups=["not", "a", "mapping"],
            )

        with pytest.raises(InputError, match="dependency_discount"):
            portfolio_voi(
                portfolio_specification=portfolio_spec,
                study_value_calculator=simple_value_calculator,
                optimization_method="dynamic_programming",
                dependency_discount=1.5,
            )

    def test_portfolio_voi_zero_cost_studies(self) -> None:
        ...
        assert (
            result["total_value"] >= 1000.0
        )  # At least the value of the zero-cost study
        # The cost should reflect selected studies
        assert result["total_cost"] >= 0

    def test_portfolio_voi_negative_value_study(self) -> None:
        ...
        study1 = PortfolioStudy(
            "Study 1", design1, cost=50000
        )  # Normal study with positive value
        study2 = PortfolioStudy(
            "Study Negative", design1, cost=60000
        )  # Study with negative value
        studies = [study1, study2]
    ...
    def test_portfolio_voi_ratio_sorting(self) -> None:
        ...
        studies = [
            ...
            study_high,
            study_med,
        ]  # Order shouldn't matter for ratio sorting
        ...
        assert (
            result["total_value"] == 1000.0
        )  # Should be the value of the high ratio study

    def test_portfolio_voi_budget_constraints(self) -> None:
        ...
        assert (
            "Medium Study" in selected_names_med or "Large Study" in selected_names_med
        )
    ...
    def test_portfolio_voi_multiple_selection_scenarios(self) -> None:
        ...
        designs = [
            ...
            ),  # Cost 10000, Base value 400
            TrialDesign(
                [DecisionOption("Treatment B", 100)]
            ),  # Cost 20000, Base value 800
            TrialDesign(
                [DecisionOption("Treatment C", 75)]
            ),  # Cost 15000, Base value 600
            TrialDesign(
                [DecisionOption("Treatment D", 30)]
            ),  # Cost 5000, Base value 240
        ]
        studies = [
    ...
    def test_portfolio_voi_zero_budget(self) -> None:
    ...
    def test_portfolio_voi_single_study_scenarios(self) -> None:
