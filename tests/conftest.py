...
def sample_nb_data_array_2strat() -> np.ndarray:
    """Provides a sample NumPy array of net benefits (5 samples, 2 strategies)."""
    return np.array([
        [100, 105],
        [110, 100],
        [90, 110],
...
def sample_net_benefit_array_2strat(sample_nb_data_array_2strat: np.ndarray) -> NetBenefitArray:
    """Provides a NetBenefitArray instance (5 samples, 2 strategies)."""
...
def sample_psa_data_dict_2params() -> dict:
    """Provides a sample dictionary of PSA parameters (3 samples, 2 parameters)."""
    return {
...
def sample_psa_2params(sample_psa_data_dict_2params: dict) -> PSASample:
    """Provides a PSASample instance (3 samples, 2 parameters)."""
    return PSASample(parameters=sample_psa_data_dict_2params)

@pytest.fixture(scope="session")
def sample_trial_design_2arms() -> TrialDesign:
    """Provides a sample TrialDesign instance with two arms."""
...
def evppi_test_data_simple():
    """
    Generates a simple dataset for testing EVPPI.
    ...
    return {
        "nb_values": nb_values,
        "p_samples": p_voi,
        "expected_evpi_approx": max(0.0, expected_evpi_val) # Ensure non-negative due to MC
    }
