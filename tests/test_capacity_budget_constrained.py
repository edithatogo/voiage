...
def test_constraints_and_names_are_validated() -> None:
    with pytest.raises(Exception):
        value_of_capacity_budget_constrained(
            np.ones((2, 2)), strategy_costs=[1.0], strategy_capacity=[1.0, 1.0], budget=1, capacity=1
        )
    with pytest.raises(Exception):
        value_of_capacity_budget_constrained(
            np.ones((2, 2)), strategy_costs=[2.0, 2.0], strategy_capacity=[1.0, 1.0], budget=1, capacity=1
        )
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"scenario_values": [[1.0, float("nan")]], "strategy_costs": [1, 1], "strategy_capacity": [1, 1], "budget": 1, "capacity": 1},
        {"scenario_values": [[1.0, 2.0]], "strategy_costs": [-1, 1], "strategy_capacity": [1, 1], "budget": 1, "capacity": 1},
        {"scenario_values": [[1.0, 2.0]], "strategy_costs": [1, 1], "strategy_capacity": [1, 1], "budget": -1, "capacity": 1},
        {"scenario_values": [[1.0, 2.0]], "strategy_costs": [1, 1], "strategy_capacity": [1, 1], "budget": 1, "capacity": 1, "information_cost": -1},
        {"scenario_values": [[1.0, 2.0]], "strategy_costs": [1, 1], "strategy_capacity": [1, 1], "budget": 1, "capacity": 1, "strategy_names": ["one"]},
    ],
)
...
def test_decision_analysis_wrapper_exposes_method() -> None:
    result = DecisionAnalysis(np.ones((2, 2))).value_of_capacity_budget_constrained(**_inputs())
    assert result.selected_strategy == "small"
