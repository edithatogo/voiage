...
static int exercise_contract() {
    ...
    std::uint64_t evpi_envelope_size = 0;
    if (voiage_v1_evpi_result_json(
            reinterpret_cast<const std::uint8_t *>(evpi_envelope),
            sizeof(evpi_envelope) - 1, nullptr, 0,
    ...
    if (voiage_v1_evppi_result_json(
            reinterpret_cast<const std::uint8_t *>(evppi_envelope),
            sizeof(evppi_envelope) - 1, nullptr, 0,
            &scalar_size) != VOIAGE_V1_STATUS_OK ||
        voiage_v1_evsi_result_json(
            reinterpret_cast<const std::uint8_t *>(evsi_envelope),
            sizeof(evsi_envelope) - 1, nullptr, 0,
            &scalar_size) != VOIAGE_V1_STATUS_OK ||
        voiage_v1_enbs_result_json(
            reinterpret_cast<const std::uint8_t *>(enbs_envelope),
            sizeof(enbs_envelope) - 1, nullptr, 0,
            &scalar_size) != VOIAGE_V1_STATUS_OK) {
        return 19;
    }
    ...
    if (voiage_v1_statistical_assurance_json(
            reinterpret_cast<const std::uint8_t *>(assurance_envelope),
            sizeof(assurance_envelope) - 1, nullptr, 0, &scalar_size) !=
        VOIAGE_V1_STATUS_OK) {
        return 21;
    }
