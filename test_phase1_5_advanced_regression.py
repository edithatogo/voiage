...
def test_advanced_regression_integration():
    ...
    for config in test_configs:
        ...
        if config["complexity"] == "linear":
            ...
            for j in range(config["n_strategies"]):
                nb_array[:, j] = sum(
                    param_samples[f"param_{i}"][j] * np.random.normal(0.5, 0.1)
                    for i in range(config["n_params"])
    ...
    # Phase assessment
    if (
        avg_evpi_diff < 1e-3 and avg_jax_time < avg_numpy_time * 2
    ):  # Allow some overhead
        assessment = "✅ SUCCESS"
...
def test_advanced_regression_models():
    ...
    for name, model in models.items():
        ...
        try:
            ...
            try:
                ...
                print(
                    f"   Uncertainty: Available (mean var = {jnp.mean(var_pred):.4f})"
                )
            except:
    ...
    try:
        pipeline = AdvancedRegressionPipeline(
            model_type="ensemble", feature_selection=True
        )
    ...
    for name, result in model_results.items():
        if result["r2_score"] > 0:
            print(
                f"   {name}: R² = {result['r2_score']:.4f}, Time = {result['fit_time']:.4f}s"
            )
...
if __name__ == "__main__":
    ...
    if success1 and success2:
        ...
        print("🎯 Ready for production use in voiage EVPPI/EVSI calculations")
    else:
