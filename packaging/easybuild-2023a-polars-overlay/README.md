# Polars 1.42.1 provider candidate for foss 2023a

This overlay resolves the split Polars Python package and native
`polars-runtime-32` provider for the Voiage foss 2023a graph. EasyBuild 5.4.0
...
The source-preparation receipt rechecks all 535 declared archive bytes from a
read-only cache created for the reviewed 2024a sibling. It binds those bytes to
this generation's recipe and records EasyBuild's `.tar.gz` crate filename
contract; it is not installation evidence.

Run the dependency-resolution check with EasyBuild 5.4.0, Environment Modules
and the pinned catalogue:

```console
eb packaging/easybuild-2023a-polars-overlay/2023a/polars-1.42.1-GCCcore-12.3.0.eb \
  --dry-run \
  --robot=packaging/easybuild-2023a-polars-overlay/2023a:packaging/easybuild-2023a-rust-overlay/2023a:packaging/easybuild-2023a-overlay/2023a:PINNED_EASYCONFIGS
```
