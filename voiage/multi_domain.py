...
class MultiDomainVOI:
    ...
    def __init__(self, domain_type: DomainType, domain_params: object):
        """
        ...
        self.domain_params: object = domain_params
        self.decision_analysis: DecisionAnalysis | None = None
        self.outcome_function: Callable[..., object] | None = None
    ...
    def create_voi_analysis(
        self,
        decision_variables: jnp.ndarray,
        uncertainty_parameters: jnp.ndarray,
        decision_outcome_function: OutcomeFunction | None = None,
    ) -> DecisionAnalysis:
        """
        ...
        if decision_outcome_function is not None:
            ...
            raise_input_error("No outcome function specified")

        # Create domain-specific parameters for DecisionAnalysis
        domain_params = vars(self.domain_params)
        analysis_params = {
    ...
    def _domain_outcome_wrapper(self) -> Callable[..., object]:
        """Wrap domain outcome function for DecisionAnalysis compatibility."""

        def wrapper(decision_vars: object, **kwargs: object) -> object:
            if self.outcome_function is None:
                raise_input_error("No outcome function set")

            additional_params = cast(
                "dict[str, object]",
                getattr(self.domain_params, "additional_params", {}),
            )
            uncertainties = kwargs.get(
                "uncertainty_parameters",
                jnp.array([0.0] * len(additional_params)),
            )

            return self.outcome_function(
                decision_vars, uncertainties, self.domain_params
            )

        return wrapper

    def _set_default_outcome_function(self) -> None:
    ...
    def _manufacturing_outcome(
        self, decision_vars: jnp.ndarray, uncertainties: jnp.ndarray, params: object
    ) -> jnp.ndarray:
        """
        ...
        demand_unc, supply_unc, _quality_unc = (
            uncertainties[:3] if len(uncertainties) >= 3 else [0.1, 0.1, 0.1]
        )
        ...
        quality_cost = (
            production_qty * (1 - quality_level) * params.failure_cost_multiplier
        )
        ...
        return jnp.array(
            [final_profit, revenue, total_cost, actual_demand, quality_level]
        )

    def _finance_outcome(
        self, decision_vars: jnp.ndarray, uncertainties: jnp.ndarray, params: object
    ) -> jnp.ndarray:
        """
        ...
        market_ret, vol, corr = (
            uncertainties[:3] if len(uncertainties) >= 3 else [0.05, 0.15, 0.3]
        )

        # Calculate portfolio return with uncertainties
        expected_return = (
            params.expected_return * portfolio_alloc
            + params.risk_free_rate * (1 - portfolio_alloc)
        )
        ...
        portfolio_value = (
            params.initial_investment * (1 + actual_return) ** investment_horizon
        )

        # Transaction costs
        transaction_costs = (
            params.initial_investment * params.transaction_cost * portfolio_alloc
        )
    ...
    def _environmental_outcome(
        ...
        control_invest, monitoring, _compliance = decision_vars
        baseline_unc, _eco_response, climate_unc = (
            uncertainties[:3] if len(uncertainties) >= 3 else [0.0, 0.0, 0.0]
        )

        # Pollution reduction calculation
        baseline_pollution = params.baseline_pollution_level * (1 + baseline_unc)
        pollution_reduction = (
            control_invest * params.compliance_cost * (1 + climate_unc)
        )
        ...
        # Monitoring value
        monitoring_value = (
            monitoring * params.monitoring_frequency * 1000
    ...
    def _engineering_outcome(
        ...
        material_prop, _load_cond, _failure_mode = (
            uncertainties[:3] if len(uncertainties) >= 3 else [0.0, 0.0, 0.0]
        )
        ...
        maintenance_reliability = 1 - jnp.exp(
            -maintenance * params.maintenance_cost_rate
        )

        system_reliability = (
            base_reliability * safety_reliability * maintenance_reliability
        )
        ...
        maintenance_cost = (
            maintenance * params.maintenance_cost_rate * params.initial_investment
            if hasattr(params, "initial_investment")
            else maintenance * 1000
        )
        ...
        return jnp.array(
            [system_value, system_reliability, total_cost, performance, failure_cost]
        )

    def _generic_outcome(
        self, decision_vars: jnp.ndarray, uncertainties: jnp.ndarray, params: object
    ) -> jnp.ndarray:
        """Handle unspecified domains with a simple linear outcome."""
        # Simple linear outcome function
        return jnp.dot(decision_vars, jnp.ones_like(decision_vars)) + jnp.sum(
            uncertainties
        )

    def _convert_to_manufacturing_params(
        self, params: DomainParameters
    ...
    def _convert_to_environmental_params(
        self, params: DomainParameters
    ...
    def _convert_to_engineering_params(
        self, params: DomainParameters
    ...
    def domain_specific_evpi(
        self, decision_analysis: DecisionAnalysis, domain_objective: str = "profit"
    ) -> dict[str, float]:
        """
        ...
        evpi = float(decision_analysis.evpi())
        ...
        if self.domain_type == DomainType.FINANCE:
            return {
                "evpi": evpi,
        ...
        if self.domain_type == DomainType.ENVIRONMENTAL:
            return {
                "evpi": evpi,
        ...
        if self.domain_type == DomainType.ENGINEERING:
            return {
                "evpi": evpi,
    ...
    def create_domain_report(
        self, decision_analysis: DecisionAnalysis
    ) -> dict[str, object]:
        """
        ...
        """
        if decision_analysis is None:
            raise_input_error("No decision analysis to report on")

        # Basic VOI metrics
        calculate_evpi = decision_analysis.calculate_evpi
        evpi = float(calculate_evpi())
        evppi_method = decision_analysis.evppi
        evppi = float(evppi_method([]))  # Assuming first parameter is most important
        ...
        domain_insights = self._generate_domain_insights(
            decision_analysis, domain_metrics
        )
    ...
    def _generate_domain_insights(
        self, decision_analysis: DecisionAnalysis, metrics: dict[str, float]
    ) -> dict[str, str]:
        """Generate domain-specific insights from analysis results."""
        insights = {}

        if self.domain_type == DomainType.MANUFACTURING:
            if metrics.get("production_uncertainty_value", 0) > metrics.get(
                "quality_uncertainty_value", 0
            ):
                insights["primary_focus"] = (
                    "Production planning uncertainty has the highest value - focus on demand forecasting and supply chain optimization."
                )
            else:
                insights["primary_focus"] = (
                    "Quality control uncertainty dominates - invest in better quality measurement and control systems."
                )

        elif self.domain_type == DomainType.FINANCE:
            if metrics.get("market_uncertainty_value", 0) > 0.5:
                insights["market_advice"] = (
                    "High market uncertainty value suggests the need for better market analysis and dynamic portfolio management."
                )

        elif self.domain_type == DomainType.ENVIRONMENTAL:
            insights["policy_advice"] = (
                "Environmental impact uncertainty suggests need for better monitoring and ecosystem response modeling."
            )

        elif self.domain_type == DomainType.ENGINEERING:
            insights["design_advice"] = (
                "System reliability uncertainty indicates need for better material characterization and load analysis."
            )
...
def calculate_domain_evpi(
    decision_analysis: DecisionAnalysis, domain_type: DomainType
) -> float:
    """Calculate EVPI for a specific domain type."""
    return float(decision_analysis.evpi())
...
def compare_domain_performance(analyses: list[MultiDomainVOI]) -> dict[str, float]:
    ...
    for _i, analysis in enumerate(analyses):
        ...
        if analysis.decision_analysis is not None:
            evpi = analysis.decision_analysis.evpi()
