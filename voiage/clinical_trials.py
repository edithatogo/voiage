...
class TrialOutcome:
    ...
    cost_effectiveness_ratio: float | None = None
    incremental_qaly: float | None = None
    net_monetary_benefit: float | None = None
    probability_cost_effective: float | None = None

    # Adaptive trial outcomes
    adaptation_triggered: bool = False
    adaptation_type: AdaptationRule | None = None
...
class AdaptiveTrialOptimizer:
    ...
    def optimize_adaptation_thresholds(self,
        ...
        if adaptation_rule == AdaptationRule.EARLY_FUTILITY:
            return self._optimize_early_futility_threshold(treatment)
        if adaptation_rule == AdaptationRule.SAMPLE_SIZE_REESTIMATION:
...
class ClinicalTrialDesignOptimizer:
    ...
    def _simulate_health_economic_outcomes(self,
        ...
        cer = cost_diff / qaly_diff if qaly_diff > 0 else float("inf")
        ...
        return float(cer), float(qaly_diff), float(nmb), float(prob_ce)
