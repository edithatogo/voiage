...
class HealthState:
    ...
    transition_probabilities: dict[str, float] | None = (
        None  # Probabilities to other states
    )
...
class HealthEconomicsAnalysis:
    ...
    def __init__(self, willingness_to_pay: float = 50000.0, currency: str = "USD"):
        ...
        self.transition_matrix: jnp.ndarray | None = None
    ...
    def calculate_qaly(
        ...
        # Apply discounting
        utility_stream * jnp.exp(-discount_rate * time_points)
        ...
        if discount_rate == 0:
            ...
            qaly = (
                health_state.utility
                * (1 - jnp.exp(-discount_rate * time_horizon))
                / discount_rate
            )

        return float(jnp.minimum(qaly, time_horizon))  # Cap at time horizon
    ...
    def calculate_cost(
        ...
        # Apply discounting
        cost_stream * jnp.exp(-discount_rate * time_points)
        ...
        return float(jnp.minimum(total_cost, time_horizon * health_state.cost))

    def calculate_icer(
        self,
        treatment1: Treatment,
        treatment2: Treatment | None = None,
        health_states1: list[HealthState] | None = None,
        health_states2: list[HealthState] | None = None,
    ) -> float:
        """
    ...
    def calculate_net_monetary_benefit(
        self, treatment: Treatment, health_states: list[HealthState] | None = None
    ) -> float:
        """
        ...
        # NMB = (QALY * WTP) - Cost
        return (qaly * self.willingness_to_pay) - cost

    def create_cost_effectiveness_acceptability_curve(
        self,
        treatment: Treatment,
        health_states: list[HealthState] | None = None,
        ...
        def nmb_for_wtp(wtp: float) -> float:
    ...
    def probabilistic_sensitivity_analysis(
        ...
        utility_multipliers = (
            random.normal(key, (num_simulations,)) * utility_uncertainty + 1.0
        )
        cost_multipliers = (
            random.normal(key, (num_simulations,)) * cost_uncertainty + 1.0
        )

        # Calculate outcomes for each simulation
        def simulate_once(i: int) -> jnp.ndarray:
            health_state = HealthState(
                state_id="temp",
                description="Temporary state",
                utility=jnp.maximum(
                    0, treatment.effectiveness * utility_multipliers[i]
                ),
                cost=treatment.cost_per_cycle
        ...
        return {
            ...
            "icer_distribution": {
                ...
                "q025": float(
                    jnp.percentile(icer_samples[icer_samples != float("inf")], 2.5)
                ),
                "q975": float(
                    jnp.percentile(icer_samples[icer_samples != float("inf")], 97.5)
                ),
            },
    ...
    def _create_default_health_states(self, treatment: Treatment) -> list[HealthState]:
        ...
        if treatment.effectiveness > 0.5:
    ...
    def create_voi_analysis_for_health_decisions(
        self,
        ...
        analysis.decision_function = lambda params, **kwargs: (
            self._health_decision_outcomes(
    ...
    def _health_decision_outcomes(
        self,
        treatments: list[Treatment],
        decision_function: Callable[..., object],
        **kwargs: object,
    ) -> list[object]:
        """Calculate custom decision outcomes for health economics."""
        outcomes = []
...
def calculate_icer_simple(
    cost_intervention: float,
    effect_intervention: float,
    cost_comparator: float = 0.0,
    effect_comparator: float = 0.0,
) -> float:
    """Calculate the simple ICER for two interventions."""
...
def calculate_net_monetary_benefit_simple(
...
def qaly_calculator(
    life_years: float, utility_weight: float, discount_rate: float = 0.03
) -> float:
    """Calculate the simple QALY value."""
    ...
    return float(min(discounted_qaly, life_years))
