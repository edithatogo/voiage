...
def bayesian_adaptive_trial_simulator(
    ...
    data_estimate_te = np.random.normal(true_te, 0.01, n_samples)
    posterior_mean_te = (
        prior_precision_te * treatment_effect + data_precision_te * data_estimate_te
    ...
    data_estimate_cr = np.random.normal(true_cr, 0.01, n_samples)
    posterior_mean_cr = (
        prior_precision_cr * control_rate + data_precision_cr * data_estimate_cr
    ...
    # Calculate updated treatment outcomes and control outcomes for net benefit
    # In Bayesian model, net benefit is calculated based on updated beliefs
    # We should use `updated_treatment_effect` and `updated_control_rate`

    # Ensure rates are between 0 and 1
    _ = np.clip(updated_control_rate + updated_treatment_effect, 0, 1)
    _ = np.clip(updated_control_rate, 0, 1)
