...
7. Release in stages: v1.1 stable core, v1.2 supported extensions, and v1.3
   experimental parity.
8. Treat VOIAGE as reusable decision-value middleware for industry decisions:
   a portable Decision Problem, risk-sensitive and constrained VOI,
   policy/uplift and model value, dependent information-source and experiment
   portfolios, forecast/signal value, flexibility, implementation, stakeholder
   perspectives, and decision-correctness assurance.
9. Provide adoption infrastructure without duplicating numerical engines:
   Decision Registry cards, a local-first Decision Studio, domain
   template/adapter registry, optional enterprise integrations, and executable
   churn plus broader consulting/business example packs.
10. Execute the comprehensive open-source and commercial VOI software
    landscape track before adopting its recommendations. Record product,
    schema, method, feature, subfeature, option, evidence, maturity, integration,
    workflow, assurance, UX and commercial-operating-model observations; map
    VOIAGE gaps and lessons; present a bounded improvement proposal for named
    human review; and only then update the roadmap in a separate approved
    change.
...
2. Every included external feature and literature method is implemented or has
   an evidence-backed disposition; commercial and hosted products are
   evidence-bounded when source, access, pricing or internal implementation is
   unavailable.
...
6. Documentation, contributor statements, and AI disclosures match executable
   evidence.
7. GitHub #314, #315, and #318--#322 own their native industry/adoption
   subissues; Project 28 fields and useful delivery, priority, risk, review and
   next-track views match Conductor metadata.
8. Landscape recommendations are review artifacts until the maintainer accepts
   them into a later roadmap change.

## Out of scope

Automatic publication, external registry acceptance, paid-service purchase,
credentialed product access, confidential-data upload, hardware purchases,
authorship decisions, automatic roadmap adoption, and unverifiable
proprietary-tool parity.
