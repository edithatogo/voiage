...
stateDiagram-v2
    [*] --> Preparation
    Preparation --> Frozen: exact packet and verified hashes
    Frozen --> Review: eligible reviewers commissioned
    Review --> Remediation: findings or substantive dissent
    Remediation --> Frozen: new candidate supersedes old packet
    Review --> ScientificExperimental: acceptable exact candidate
    Review --> ReviewedExclusion: generic or scoped capability excluded
    ScientificExperimental --> MaintainerDecision: current human receipts
    MaintainerDecision --> RetainExperimental
    MaintainerDecision --> FutureImplementationTrack
    ReviewedExclusion --> Unsupported
    Preparation --> Unsupported: reviewer or evidence unavailable
    ScientificExperimental --> Preparation: expiry or material delta
    RetainExperimental --> Preparation: expiry or material delta
    FutureImplementationTrack --> Preparation: scientific scope changes
```
