...
Define how `voiage` should participate in a health economics and outcomes
research (HEOR) ecosystem without becoming a dependency sink or absorbing
non-VOI concerns. `voiage` should remain the Value of Information engine for
HEOR workflows. It should integrate with upstream health-economic model
generators and downstream reporting or workflow tools through portable
artifacts, schemas, adapters, and conformance fixtures.
...
- `innovate` owns health-intervention uptake, implementation diffusion, policy
  adoption, and network spread models that can inform implementation scenarios.
...
- Define `voiage` as a stable VOI consumer and producer in the HEOR ecosystem.
...
- Define how `voiage` can use `mars` as a metamodel backend without changing
  the `mars` API.
- Define how `innovate` outputs could become inputs to HEOR VOI workflows when
...
- Require future sibling modules to have a clear HEOR role before integration.
...
- Generic non-HEOR modelling concerns are out of scope for this ecosystem plan.
