...
- [ ] Task: Define dependency policy for integration consumers.
  - [ ] Keep `lifecourse` out of `voiage` runtime dependencies.
  - [ ] Recommend that `lifecourse` uses an optional `voiage` extra only after version and dependency policy are stable.
  - [ ] Consider splitting heavy `voiage` dependencies into extras if the integration needs a lighter core install.
- [ ] Task: Define stable result object needs.
  - [ ] Ensure EVPI, EVPPI, EVSI, and ENBS results can carry method settings, scaling metadata, diagnostics, package version, and provenance.
  - [ ] Keep result payloads compatible with existing core API result schemas.
...
- [x] Task: Add conformance expectations.
  ...
  - [ ] Add EVSI and ENBS expectations only once study-design fields are agreed.
  - [x] Document tolerances and method settings.

## Phase 5: Release And Documentation Coordination [checkpoint: ]

- [ ] Task: Define compatibility versioning.
  - [ ] Record supported `voiage` and `lifecourse` contract versions.
  - [ ] Record the HEOML profile version that the fixture set targets.
  - [ ] Require changelog notes for public contract changes.
- [ ] Task: Add user and contributor documentation.
  - [ ] Explain artifact exchange, optional adapter usage, limitations, and experimental method status.
  - [ ] Document how downstream packages and bindings should validate against the shared fixtures.
  - [ ] Link the shared profile and run-bundle docs back to `heoml/`.
