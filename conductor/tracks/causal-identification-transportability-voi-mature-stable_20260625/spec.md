# Track Specification: Causal Identification Transportability And External Validity VOI Mature Stable Path

## Overview

Implement and promote causal-identification, transportability, and external-validity VOI from contract scaffolds through mature/stable runtime status.

This dedicated method-family track covers causal-identification, transportability, external-validity VOI all the way from runtime implementation through mature/stable promotion review. It exists because the prior adjacent-frontier umbrella track was too broad to prove each method family independently.
...
1. Define runtime APIs for target-population causal evidence, effect transport, external-validity diagnostics, and decision impact.
2. Add deterministic synthetic fixtures and at least one open-data example with source, license, transform, and hash metadata.
3. Implement Python runtime and Rust-kernel parity where numerically appropriate, with documented approximation policy otherwise.
...
`causal-identification`, `transportability`, `external-validity`, `target population`, `stable promotion`
