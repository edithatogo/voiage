...
| Issue | Current unresolved requirement | Next action |
| --- | --- | --- |
...
- **pyOpenSci:** declarations are confirmed. Complete the actual survey and
  supply human-written text. The retained AI-assisted draft is reference
  material, not proof of human authorship. The earlier two requests are closed.
- **Journal sequence:** continue pyOpenSci first and eligible JOSS, then arXiv
  in line with the maintainer-reported direction. Defer separate R Journal/JSS
  articles until a distinct contribution exists.
- **Sampling-harm review:** either commission the required qualified reviews
  for a narrow candidate or retain the proposed exclusion for the same
  governed review. Deferral is also possible, but is not scientific acceptance.
- **Sustainability:** defer OSC activation and NumFOCUS application. A genuine
  shared-ownership decision, verified funding account, or reopened application
  process would change readiness; a local document does not.
- **Badges and identifiers:** certify only supported questionnaire answers and
  use only assigned identifiers. Do not change the solo-maintainer review model
  merely to manufacture a perfect numerical score.

Technical work continues without another permission request. These remaining
requirements concern actual evidence, personal actions or accountable choices;
they are not requests to repeat previously confirmed declarations.
