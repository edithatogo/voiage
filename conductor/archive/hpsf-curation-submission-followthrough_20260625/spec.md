# Track Specification: HPSF Curation Submission Follow-Through

## Overview

Prepare HPSF listing or curation submission evidence and track external review status.
...
Docs audit, community metadata audit, gh, and browser only for submission portals.
...
1. Validate governance, security, support, release, and reproducibility materials against HPSF expectations.
2. Prepare a submission packet with package status, supported registries, and evidence links.
3. Pause before portal submission or account-bound action.
4. Track curation review, requested changes, approval, rejection, or blocked status explicitly.
...
1. HPSF evidence packet exists with current registry and CI links.
2. External review status is represented without implying approval.
3. Follow-up changes are split into concrete Conductor tasks if reviewers request them.

## Required Keywords For Validation

`HPSF`, `curation`, `submission`, `external review`, `blocked`
