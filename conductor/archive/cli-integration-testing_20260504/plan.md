...
### 2.1 Remove test ignores incrementally [DONE]
- [x] **Pass 1:** Remove ignores for tests that should now pass (after Tracks 1-2):
  - `test_sample_information.py`
  - `test_plotting.py`
  - `test_structural.py`
  - `test_network_nma.py`
  - `test_network_meta_analysis.py`
  - `test_adaptive_comprehensive.py`
  - `test_portfolio_comprehensive.py`
  - `test_sequential.py`
  - `test_backends.py`
  - `test_analysis_comprehensive.py`
- [x] Run each test file individually — fix failures before moving to next
- [x] **Pass 2:** Remove ignores for remaining comprehensive tests
- [x] **Pass 3:** Remove ignores for domain-specific tests (financial, environmental, healthcare)
- [x] **Pass 4:** Remove ignores for CLI, coverage, and utility tests
  - Verified by the absence of `--ignore` patterns in pytest config and a full-suite run.

### 2.2 Fix failing tests [DONE]
...
- [x] Fix tests in priority order: core method tests first, then integration, then domain-specific
  - Remaining coverage and branch gaps were closed with focused CLI/internal regression tests, and the full suite is now green again.

### 2.3 Create missing integration tests [DONE]
- [x] `tests/test_full_workflow.py` — end-to-end: load data → run EVPI → run EVPPI → run EVSI → plot
- [x] `tests/test_decision_analysis_methods.py` — test all DecisionAnalysis methods with realistic data
- [x] `tests/test_backend_consistency.py` — verify NumPy and JAX backends produce consistent results and the CLI EVPI workflow matches the backend output
- [x] `tests/test_cross_domain.py` — test healthcare, financial, environmental domain analyses
- [x] `tests/test_cli_e2e.py` — comprehensive CLI end-to-end tests

### 2.4 Property-based and fuzzing tests [PENDING]
- [x] Add hypothesis property-based tests for:
  - EVPI is always ≥ 0
  - EVPPI ≤ EVPI for any parameter subset
  - EVSI → 0 as sample size → 0
  - ENBS = EVSI - cost (identity)
  - Dominance is transitive
  - Implemented in `tests/test_property_invariants.py` and `tests/test_sample_information.py`.
- [x] Add fuzzing tests for CLI inputs (malformed JSON, empty files, extreme values)
  - Implemented in `tests/test_cli_invariants.py`.
...
- [x] Add `tests/benchmarks/` directory with pytest-benchmark tests
- [x] Benchmark EVPI performance at varying sample sizes (100, 1000, 10000, 100000)
- [x] Benchmark EVPPI at varying parameter counts
- [x] Benchmark EVSI (two-loop vs regression vs efficient) for speed and accuracy
- [x] Benchmark JAX vs NumPy for each method
- [x] Store baseline results for regression detection
  - Stored in `tests/regression_data/core_metrics.json`.

### 3.2 Add regression test suite [DONE]
- [x] Save reference results to `tests/regression_data/` (known-good outputs for fixed inputs)
- [x] Compare current outputs against reference with tolerance
- [x] Run on every CI build to detect performance regressions
  - Wired into `.github/workflows/ci.yml` as `benchmark-regression`.

---

## Phase 4: Full Verification [checkpoint: ]

### 4.1 Run complete tooling suite [DONE]
- [x] `ruff check voiage/ tests/` — zero errors
- [x] `tox -e typecheck` — zero errors
- [x] `pytest tests/ --cov=voiage --cov-fail-under=90` — all tests pass, coverage ≥90%

### 4.2 Verify no ignored test files remain [DONE]
- [x] Count `--ignore` patterns in pytest config — target: 0 (or at most a handful of justified exclusions)
- [x] All test files should either pass or be genuinely deleted (not just ignored)
  - Verified by the current pytest configuration and the full-suite run.

### 4.3 Verify CLI completeness [x]
- [x] Every method in `voiage.methods` has a corresponding CLI command
- [x] Every plotting function has a CLI command
- [x] `voiage --help` shows all commands organized by category
