# Track Implementation Plan: Data Quality Measurement Privacy And Linkage VOI Mature Stable Path

## Phase 1: Contract And Maturity Boundary [checkpoint: ]

- **Legacy follow-up (not part of completed track acceptance):** Audit existing contract scaffolds, docs, and runtime surfaces for this method family.
...
- **Legacy follow-up (not part of completed track acceptance):** Implement or extend Python runtime APIs, result objects, CLI commands, and deterministic synthetic fixtures.
    - **Legacy follow-up (not part of completed track acceptance):** Preserve evidence links, commands, artifact paths, blocked gates, and maturity status.
    - **Legacy follow-up (not part of completed track acceptance):** Preserve commit notes, git notes, short commit SHA updates, and plan-update commits.
...
- Implemented the fixture-backed Python runtime and result envelope in
  ``voiage/methods/data_quality.py`` and exposed it through the curated package
  API and ``calculate-data-quality`` CLI command.
- Updated the v1 schemas and examples to ``fixture-backed`` maturity and added
  hash-pinned evidence in ``specs/frontier/data-quality/v1/fixtures/evidence.json``.
- Added runtime, contract, CLI, export, and registry tests. Local lint,
  ...
  #169 passed all required jobs and Python CodeQL.
...
- Hosted PR #169 merged as ``e9e5baf`` with required checks and Python CodeQL
