...
The initial evaluation is preserved. The strengthened replay receipt,
`evaluation-verified.json`, verifies the exact public macOS wheel SHA-256 and
all 192 installed payload files before importing voiage. Installer-managed
RECORD and generated bytecode caches are not original wheel payload evidence.
...
Download the matching wheel from the public v2.2.0 release or PyPI and provide
its local path. Only the three reviewed public wheel hashes are accepted;
same-version local builds and modified installed payloads are rejected.
...
/path/to/consumer/.venv/bin/python -I /path/to/voiage/scripts/run_vop_research_handoff.py evaluate --export-receipt /path/to/new-run/export/export.json --export-sha256 <observed-export-sha256> --wheel /path/to/public/voiage-2.2.0-platform.whl --output /path/to/new-run/evaluation.json
```
...
draw count, finite values, reviewed wheel digest, installed payload bytes,
voiage version and numerical reference.
...
without regenerating a machine-specific export receipt. The CSV can also be
regenerated from the pinned public VOP parameters. No new venue communication
or human attestation was produced by this replay.
