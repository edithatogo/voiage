...
def realistic_obs_modeler(psa_samples, study_design, bias_models):
    """Simulate a realistic observational study.
    ...
    effectiveness_params = psa_samples.parameters.get(
        "effectiveness", np.random.normal(0.7, 0.1, n_samples)
    )
    cost_params = psa_samples.parameters.get(
    ...
    biased_effectiveness = effectiveness_params * (
        1 - confounding_strength
    ...
    final_effectiveness = biased_effectiveness + np.random.normal(
        0, selection_bias_prob * 0.1, n_samples
    )
    ...
    treatment_benefit = (
        final_effectiveness * 20000
    )  # Convert effectiveness to monetary benefit
...
def sophisticated_observational_modeler(psa_samples, study_design, bias_models):
    """Demonstrate bias adjustment in a sophisticated observational study.
    ...
    selection_retention = np.clip(1.0 - selection_bias_prob, 0.0, 1.0)
    observed_effectiveness = effectiveness * (
        1 - confounding_strength
    ) * selection_retention + np.random.normal(0, measurement_error, n_samples)
    ...
    adjusted_effectiveness = observed_effectiveness * (
        1 + adjustment_factor * confounding_strength
...
def main():
    ...
    params = {
        "effectiveness": np.random.normal(
            0.7, 0.1, n_samples
    ...
    annuity = (1 - (1 + dr) ** -time_horizon) / dr if dr > 0 else float(time_horizon)
    ...
    # Low bias scenario
    low_bias_models: dict[str, object] = {
    ...
    # High bias scenario
    high_bias_models: dict[str, object] = {
