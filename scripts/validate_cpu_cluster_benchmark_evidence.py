"""Validate CPU cluster and distributed benchmark evidence packets."""
...
REQUIRED = {
    ...
    "speedup",
    "claim_level",
    "result_envelope_preserved",
    "diagnostics_preserved",
}
STATUSES = {"passed", "blocked", "not_available", "failed"}
CLAIM_LEVELS = {"reference", "smoke", "production_speedup"}
...
def validate_manifest(path: Path) -> dict[str, Any]:
    ...
    for entry in packets:
        ...
        if not isinstance(packet["worker_count"], int) or packet["worker_count"] < 1:
            raise ValueError(f"{artifact}: worker_count must be positive")
        if not isinstance(packet["node_count"], int) or packet["node_count"] < 1:
            raise ValueError(f"{artifact}: node_count must be positive")
        if (
        ...
        if packet["status"] == "passed":
            ...
            if packet["cpu_baseline_seconds"] <= 0:
                raise ValueError(f"{artifact}: passed packets require a CPU baseline")
            if (
                not packet["result_envelope_preserved"]
                or not packet["diagnostics_preserved"]
            ):
                raise ValueError(
                    f"{artifact}: passed packets require contract preservation"
                )
            if packet["claim_level"] == "production_speedup" and (
                packet["workload_scale"] != "production" or packet["speedup"] <= 1
            ):
                raise ValueError(
                    f"{artifact}: production speedup requires production workload and speedup > 1"
                )
