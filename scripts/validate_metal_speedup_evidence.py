"""Validate Apple Metal/MPS benchmark packets and CPU fallback evidence."""
...
def validate_manifest(path: Path) -> dict[str, Any]:
    ...
    for entry in packets:
        ...
        if packet["backend"] not in {"cpu", "metal"}:
            raise ValueError(f"{artifact}: backend must be cpu or metal")
        ...
        if packet["status"] == "passed":
            ...
            if packet["backend"] == "metal" and packet["speedup"] <= 1:
                raise ValueError(
                    f"{artifact}: Metal production packet requires speedup > 1"
                )
            if (
