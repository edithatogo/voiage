...
from pyvoi.core.data_structures import PortfolioSpec, PortfolioStudy, NetBenefitArray
    ...
    **kwargs: Any
    ...
    if optimization_method == "greedy":
        ...
        for study in portfolio_specification.studies:
            ...
            cost = study.cost
            if cost <= 0: # Avoid division by zero, treat as infinitely good or handle as error
            ...
            studies_with_values.append({'study': study, 'value': value, 'cost': cost, 'ratio': ratio})
        ...
        for item in studies_with_values:
            ...
            if budget is None or (current_total_cost + study_cost <= budget):
                ...
                current_total_value += study_value # Summing individual EVsIs is usually okay
...
if __name__ == '__main__':
    from pyvoi.core.data_structures import TrialDesign, TrialArm # For dummy data
    ...
    def dummy_calculator(study: PortfolioStudy) -> float:
        if "Alpha" in study.name: return 100.0 # EVSI or ENBS
        if "Beta" in study.name: return 150.0
        if "Gamma" in study.name: return 80.0
        if "Delta" in study.name: return 120.0 # High value, high cost
        return 0.0
    ...
    studies = [
        PortfolioStudy(name="Study Alpha", design=study_A_design, cost=50),   # Ratio 100/50 = 2
        PortfolioStudy(name="Study Beta", design=study_B_design, cost=60),    # Ratio 150/60 = 2.5
        PortfolioStudy(name="Study Gamma", design=study_C_design, cost=40),   # Ratio 80/40 = 2
        PortfolioStudy(name="Study Delta", design=study_D_design, cost=100),  # Ratio 120/100 = 1.2
    ]

    print("\n--- Greedy Portfolio VOI (No Budget) ---")
    portfolio_spec_no_budget = PortfolioSpec(studies=studies, budget_constraint=None)
    result_no_budget = portfolio_voi(portfolio_spec_no_budget, dummy_calculator, "greedy")
    print(f"Selected (no budget): {[s.name for s in result_no_budget['selected_studies']]}")
    print(f"Total Value: {result_no_budget['total_value']}, Total Cost: {result_no_budget['total_cost']}")
    ...
    result_budget_100 = portfolio_voi(portfolio_spec_budget_100, dummy_calculator, "greedy")
    selected_names_b100 = sorted([s.name for s in result_budget_100['selected_studies']])
    print(f"Selected (budget 100): {selected_names_b100}")
    print(f"Total Value: {result_budget_100['total_value']}, Total Cost: {result_budget_100['total_cost']}")
    ...
    result_budget_70 = portfolio_voi(portfolio_spec_budget_70, dummy_calculator, "greedy")
    selected_names_b70 = sorted([s.name for s in result_budget_70['selected_studies']])
    print(f"Selected (budget 70): {selected_names_b70}")
    print(f"Total Value: {result_budget_70['total_value']}, Total Cost: {result_budget_70['total_cost']}")
    ...
    # Test other methods (expect NotImplementedError)
    try:
        ...
        raise AssertionError("IP method did not raise NotImplementedError.")
