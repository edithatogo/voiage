...
from pyvoi.core.utils import check_input_array, check_parameter_samples
...
def evpi(
    ...
    if isinstance(nb_array, NetBenefitArray):
        ...
        raise InputError(
            "`nb_array` must be a NumPy array or NetBenefitArray object."
        )

    check_input_array(nb_values, expected_ndim=2, name="nb_array")

    if nb_values.shape[1] == 0: # No strategies
        return 0.0
    if nb_values.shape[1] == 1: # Single strategy
        return 0.0
    if nb_values.shape[0] == 0: # No samples
...
def evppi(
    ...
    if n_regression_samples is not None:
        if not isinstance(n_regression_samples, int) or n_regression_samples <= 0:
            raise InputError("n_regression_samples, if provided, must be a positive integer.")
