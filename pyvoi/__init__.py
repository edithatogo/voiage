...
"""
pyVOI: A Python Library for Value of Information Analysis.
...
# Make key data structures easily accessible from `pyvoi.core` or directly `pyvoi`
# --- Plotting Utilities (Optional Top-Level Import) ---
# Users might prefer to import these explicitly, e.g., `from pyvoi.plot import plot_ceac`
...
# The application using pyVOI can then set up its own logging handlers.
import logging

# --- Configuration and Exceptions ---
from . import (
    config,  # Allow access like `pyvoi.config.DEFAULT_DTYPE`
    exceptions,  # Allow access like `pyvoi.exceptions.InputError`
)
...
# for ease of use, e.g., `from pyvoi import evpi`.
...
# --- __all__ for `from pyvoi import *` ---
# Define what gets imported by `from pyvoi import *`.
...
# Print a message if run directly (e.g., python -m pyvoi)
# This part is optional and usually for packages with a runnable __main__.py
# if __name__ == "__main__": # pragma: no cover
#     print(f"pyVOI package version: {__version__}")
