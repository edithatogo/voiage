...
- EVPI vs. WTP
- EVPPI vs. WTP (for a specific parameter set)
- EVSI vs. Sample Size
- ENBS vs. Sample Size
- Dynamic VOI over time
"""
...
def plot_evpi_vs_wtp(
    ...
    **plot_kwargs
    ...
    """
    Plots EVPI against a range of Willingness-to-Pay (WTP) thresholds.
...
def plot_evsi_vs_sample_size(
    ...
    """
    Plots EVSI and optionally ENBS and research costs against sample sizes.
    ...
    if plot_evsi_kwargs: _plot_evsi_kwargs.update(plot_evsi_kwargs)

    _plot_enbs_kwargs = {"label": "ENBS", "color": "green", "linestyle": "--"}
    if plot_enbs_kwargs: _plot_enbs_kwargs.update(plot_enbs_kwargs)

    _plot_cost_kwargs = {"label": "Research Cost", "color": "red", "linestyle": ":"}
    if plot_cost_kwargs: _plot_cost_kwargs.update(plot_cost_kwargs)
    ...
    if enbs_values is not None or research_costs is not None:
        ...
        if enbs_values is not None:
            ...
            if len(enbs_arr) != len(ss_arr): raise InputError("Length of enbs_values mismatch.")
            # Simple heuristic for secondary axis: if max ENBS is very different from max EVSI
            if len(evsi_arr)>0 and len(enbs_arr)>0 and (np.max(np.abs(evsi_arr)) > 0 and (np.max(np.abs(enbs_arr)) / np.max(np.abs(evsi_arr)) < 0.1 or \
               np.max(np.abs(enbs_arr)) / np.max(np.abs(evsi_arr)) > 10)):
        ...
        elif enbs_values is not None or research_costs is not None : # If plotting more than EVSI on ax1
            ax1.set_ylabel(f"{ylabel_evsi} / {ylabel_enbs}")


        if enbs_values is not None:
            enbs_arr = np.asarray(enbs_values, dtype=DEFAULT_DTYPE) # Re-assert for safety
            ln2 = ax2.plot(ss_arr, enbs_arr, **_plot_enbs_kwargs) # type: ignore
            lines.extend(ln2)
            labels.extend([l.get_label() for l in ln2])

        if research_costs is not None:
            cost_arr = np.asarray(research_costs, dtype=DEFAULT_DTYPE)
            if len(cost_arr) != len(ss_arr): raise InputError("Length of research_costs mismatch.")
            ln3 = ax2.plot(ss_arr, cost_arr, **_plot_cost_kwargs) # type: ignore
            lines.extend(ln3)
            labels.extend([l.get_label() for l in ln3])
...
if __name__ == '__main__':
    if not MATPLOTLIB_AVAILABLE:
        ...
        evsi_test_vals = 2000 * (1 - np.exp(-sample_sizes_test / 200)) # EVSI increases with N
        ...
        print("--- voi_curves.py tests completed (plots generated if Matplotlib available) ---")
