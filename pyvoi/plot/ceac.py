...
"""
Plotting functions for Cost-Effectiveness Acceptability Curves (CEACs),
Cost-Effectiveness Planes, and related visualizations like EVPPI surfaces.
"""
...
from pyvoi.core.utils import calculate_net_benefit # If NMB needs to be calculated from cost/effect
from pyvoi.exceptions import PlottingError, InputError
...
def plot_ceac(
    ...
    **plot_kwargs_per_strategy: Optional[List[dict]] # List of dicts for each strategy's plot call
) -> Axes:
    """
    Plots a Cost-Effectiveness Acceptability Curve (CEAC).
    ...
    if not MATPLOTLIB_AVAILABLE:
        raise PlottingError("Matplotlib is required for plotting functions but not installed.")
    ...
    for w_idx in range(n_wtp_points):
        ...
        for s_idx in range(n_strategies):
            prob_ce[s_idx, w_idx] = np.sum(optimal_strategy_indices_at_wtp == s_idx) / n_samples

    # Plot CEAC for each strategy
    user_plot_kwargs = {}
    if plot_kwargs_per_strategy is not None:
        if isinstance(plot_kwargs_per_strategy, list) and len(plot_kwargs_per_strategy) == n_strategies:
    ...
    for s_idx in range(n_strategies):
        ...
        if 'color' not in user_plot_kwargs.get(s_idx, {}):
            ...
            current_kwargs['color'] = prop_cycle.by_key()['color'][s_idx % len(prop_cycle.by_key()['color'])]

        current_kwargs.update(user_plot_kwargs.get(s_idx, {})) # Override with user specifics
...
def plot_ce_plane(
    ...
    if scatter_kwargs: _scatter_kwargs.update(scatter_kwargs)

    _wtp_line_kwargs = {"color": "red", "linestyle": "--", "label": f"WTP = {wtp_threshold}"}
    if wtp_line_kwargs: _wtp_line_kwargs.update(wtp_line_kwargs)
    ...
    if wtp_threshold is not None:
        if not isinstance(wtp_threshold, (int, float)) or wtp_threshold < 0:
            ...
            if 'label' not in _wtp_line_kwargs or _wtp_line_kwargs['label'].startswith("WTP = "):
...
if __name__ == '__main__':
    if not MATPLOTLIB_AVAILABLE:
        ...
        for w_idx, wtp_val in enumerate(wtp_test_ceac):
            nb_test_data[:, 0, w_idx] = base_nb_s1 + 0.1 * wtp_val # Strat 1 benefits from WTP
            nb_test_data[:, 1, w_idx] = base_nb_s2 # Strat 2 is flat with WTP
            nb_test_data[:, 2, w_idx] = base_nb_s3 - 0.05 * wtp_val # Strat 3 becomes worse

        strat_names_test = ["Treatment A", "Treatment B (Standard)", "Treatment C"]

        fig_ceac, ax_ceac = plt.subplots() # type: ignore
        plot_ceac(nb_test_data, wtp_test_ceac, strategy_names=strat_names_test, ax=ax_ceac)
        ...
        delta_cost_test = np.random.normal(10000, 5000, n_samples_cep) # Incremental Costs
        ...
        print("--- ceac.py tests completed (plots generated if Matplotlib available) ---")
