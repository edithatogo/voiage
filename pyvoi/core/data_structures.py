...
class NetBenefitArray:
    ...
    def __post_init__(self):
        ...
        if self.strategy_names is not None:
            if not isinstance(self.strategy_names, list) or \
               not all(isinstance(name, str) for name in self.strategy_names):
...
class PSASample:
    ...
    """
    parameters: Union[Dict[str, np.ndarray], Any] # Using Any for xr.Dataset to avoid hard dep initially
        ...
        if isinstance(self.parameters, dict):
            ...
            for name, values in self.parameters.items():
                if not isinstance(name, str):
                    raise InputError("Parameter names in PSASample dictionary must be strings.")
                if not isinstance(values, np.ndarray):
                    raise InputError(f"Parameter '{name}' values must be a NumPy array.")
                if values.ndim != 1:
                    raise DimensionMismatchError(
            ...
            if current_n_samples == -1 : # Should not happen if dict is not empty
                 raise InputError("Could not determine n_samples from parameters dictionary.")
    ...
    def n_samples(self) -> int:
        ...
        if isinstance(self.parameters, dict):
            if not self.parameters: return 0
...
class TrialDesign:
    ...
    def __post_init__(self):
        if not isinstance(self.arms, list) or not self.arms:
            raise InputError("TrialDesign 'arms' must be a non-empty list of TrialArm objects.")
...
class PortfolioSpec:
    ...
    def __post_init__(self):
        if not isinstance(self.studies, list) or not self.studies:
            raise InputError("PortfolioSpec 'studies' must be a non-empty list of PortfolioStudy objects.")
        if not all(isinstance(study, PortfolioStudy) for study in self.studies):
            raise InputError("All elements in 'studies' must be PortfolioStudy objects.")
        study_names = [study.name for study in self.studies]
        if len(study_names) != len(set(study_names)):
            raise InputError("PortfolioStudy names within a PortfolioSpec must be unique.")

        if self.budget_constraint is not None:
            if not isinstance(self.budget_constraint, (int, float)) or self.budget_constraint < 0:
                raise InputError("PortfolioSpec 'budget_constraint' must be a non-negative number if specified.")
...
class DynamicSpec:
    ...
    """
    time_steps: Sequence[float] # Using Sequence for more flexibility (list, tuple, np.array)

    def __post_init__(self):
        if not isinstance(self.time_steps, Sequence) or not self.time_steps:
            raise InputError("'time_steps' must be a non-empty sequence (list, tuple, np.array).")
...
if __name__ == '__main__':
    ...
    try:
        PSASample(parameters={"p1": np.array([1,2]), "p2": np.array([3,4,5])}) # Mismatched lengths
    except DimensionMismatchError as e:
        print(f"Caught expected error: {e}")
    ...
    try:
        TrialDesign(arms=[TrialArm("T1", 50), TrialArm("T1", 60)]) # Duplicate arm names
