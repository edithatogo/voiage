...
from pyvoi.exceptions import InputError, FileFormatError # Assuming FileFormatError is in exceptions
...
if not hasattr(__import__('pyvoi.exceptions'), 'FileFormatError'):
    class FileFormatError(InputError):
        ...
        pass
    setattr(__import__('pyvoi.exceptions'), 'FileFormatError', FileFormatError)
...
def read_net_benefit_array_csv(
    ...
    """
    Reads a NetBenefitArray from a CSV file.
    ...
    try:
        ...
        with open(filepath, 'r', newline='') as csvfile:
            ...
            if skip_header:
                ...
                try:
                    ...
                    raise FileFormatError(f"CSV file '{filepath}' is empty or header is missing.")
            ...
            for row_idx, row in enumerate(reader):
                try:
                    ...
                    raise FileFormatError(
                        ...
                        f"Non-numeric value found. Original error: {e}"
                    )
        ...
        if final_strategy_names is not None and len(final_strategy_names) != values.shape[1]:
            raise InputError(
        ...
        if not isinstance(e, (FileFormatError, InputError)):
            raise FileFormatError(f"Failed to read or parse CSV file '{filepath}': {e}") from e
        raise
...
def write_net_benefit_array_csv(
    ...
    """
    Writes a NetBenefitArray to a CSV file.
    ...
    try:
        ...
        raise IOError(f"Failed to write NetBenefitArray to CSV file '{filepath}': {e}") from e
...
def read_psa_samples_csv(
    ...
    """
    Reads PSA samples from a CSV file into a PSASample object.
    ...
    try:
        ...
        if not isinstance(e, (FileFormatError, InputError)):
            raise FileFormatError(f"Failed to read or parse PSA samples from CSV file '{filepath}': {e}") from e
        raise
...
def write_psa_samples_csv(
    ...
    """
    Writes a PSASample object to a CSV file.
    ...
    if not param_names:
        ...
        try:
            ...
            raise IOError(f"Failed to write empty PSASample to CSV file '{filepath}': {e}") from e
    ...
    try:
        ...
        raise FileFormatError(f"Inconsistency in PSASample data during CSV write: {e}") from e
...
if __name__ == '__main__':
    ...
    nba_values = np.array([[100, 110.5], [90, 120.0], [105, 115.33]], dtype=DEFAULT_DTYPE)
    ...
    try:
        ...
        read_nba = read_net_benefit_array_csv(nba_filepath, dtype=DEFAULT_DTYPE) # strategy_names read from header

        assert np.array_equal(original_nba.values, read_nba.values), "NBA values differ after I/O."
        assert original_nba.strategy_names == read_nba.strategy_names, "NBA strategy names differ after I/O."
    ...
    try:
        ...
        read_psa = read_psa_samples_csv(psa_filepath, dtype=DEFAULT_DTYPE) # param_names read from header

        assert original_psa.n_samples == read_psa.n_samples, "PSA n_samples differ."
        assert set(original_psa.parameter_names) == set(read_psa.parameter_names), "PSA parameter names differ."
        for name in original_psa.parameter_names:
            assert np.array_equal(original_psa.parameters[name], read_psa.parameters[name]), \
        ...
        assert np.array_equal(original_psa.parameters["cost_A"], read_psa_explicit_names.parameters["p1"])
